...
NavigationCard::NavigationCard() : GameObject (obj_e::NavigationCard)
{
	m_fighters = false;
	m_rowers = false;
	m_bird = false;
...
const std::vector<character_e> & NavigationCard::getOverboard() const {
	return m_overboard;
}

const std::vector<character_e> & NavigationCard::getThirsty() const {
	return m_thirsty;
}
...
bool NavigationCard::onLoad(const rapidjson::Value & doc) {
	...
	for (unsigned i = 0; i < overboard.Size(); ++i) {
		...
		m_overboard.push_back(static_cast<character_e>(type));
	}
	
	for (unsigned i = 0; i < thirsty.Size(); ++i) {
		auto type = thirsty[i].GetInt();
		m_thirsty.push_back(static_cast<character_e>(type));
	}
